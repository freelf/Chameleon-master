...
{
    _tags = [[NSMutableArray alloc] initWithArray:items];
...
- (void)timerStart
{
	timer = [NSTimer scheduledTimerWithTimeInterval:movespeed target:self selector:@selector(autoTurnRotation) userInfo:nil repeats:YES];
...
{
    for (NSInteger i = 0; i < _tags.count; i ++) {
        [self updateFrameOfPoint:i direction:normalDirection andAngle:0.0002];
    }
...
{
    [self timerStop];
	inertia = [NSTimer scheduledTimerWithTimeInterval:movespeed target:self selector:@selector(inertiaStep) userInfo:nil repeats:YES];
...
- (void)inertiaStep
{
    if (velocity <= 0) {
        ...
        velocity -= 70.;
		CGFloat angle = velocity / self.frame.size.width * 2. * movespeed;//inertia.duration;
        for (NSInteger i = 0; i < _tags.count; i ++) {
